...
class GSplatContextManager:
    ...
    def __init__(self,
        ...
        log(bold(f'GSplatContextManager initialized with attribute dtype: {self.dtype}, texture dtype: {self.tex_dtype}, offline rendering: {self.offline_rendering}, buffer size: {init_buffer_size}, texture size: {init_texture_size}'))

        if not self.offline_rendering:
            log(bold('Using online rendering mode, in this mode, calling the rendering function of fast_gauss will write directly to the currently bound framebuffer'))
            log(bold('In this mode, the output of all rasterization calls will be None (same output count). Please do not perform further processing on them'))
            log(bold('Please make sure to set up the correct GUI environment before calling the rasterization function, see more in readme.md'))
    ...
    def resize_textures(self, H: int, W: int):  # analogy to update_gl_buffers
        ...
        if H > self.max_H or W > self.max_W:  # max got updated
            ...
            if not init: log(bold('Resizing textures to:'), int(H), int(W))
    ...
    def resize_buffers(self, v: int = 0):
        ...
        if v > self.max_verts:
            ...
            if not init: log(bold('Resizing buffers to:'), int(v))
